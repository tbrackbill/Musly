...
| GAP-10 | Cold resume restarts the track from 0 | The queue and the track are restored correctly, but the saved offset is not: setting an audio source makes `positionStream` emit 0, whose listener assigns it straight to `_position`, so the `if (_position > 0)` guard on the seek in `_prepareCurrentSong` is already false by the time it runs. Deliberately left alone — the fix touches `setAudioSource`, which carries an explicit warning about destroying the ExoPlayer AudioTrack and causing A2DP renegotiation, and getting the right track back matters far more than the offset. | Pass `initialPosition:` to `setAudioSource` rather than seeking afterwards, and verify against A2DP. |
